...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createBusiness() {
    ...
    const body = { ...req.body };
    if (body?.internet_resource?.length > 0) {
      const create_internet_resource = body.internet_resource;
      body.internet_resource = {
        create: create_internet_resource,
      };
    } else {
      delete body.internet_resource;
    }
